...
import { PageHeader, downloadExport, rupees } from '@/features/admin/_shared';
...
interface Downloads {
  ...
  invoices: { id?: string; invoice_no?: string | null; url?: string | null; amount: number }[];
}
...
function DownloadCard({
  ...
  to?: string;
  /** Authenticated API path; used for invoices that must not hit /media. */
  downloadPath?: string;
  ...
  return (
    <div className="group card flex items-center justify-between gap-4 transition hover:border-brand hover:shadow-md">
      ...
      <div className="flex shrink-0 items-center gap-1">
        {to && (
          <Link to={to} className="btn-ghost py-2" title="Open">
        ...
        {downloadPath ? (
          <button
            ...
            title="Download"
            onClick={() => downloadExport(downloadPath, {}, `${title}.pdf`)}
          >
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {certs.map((c) => (
              <DownloadCard
                ...
                note={c.remarks}
                icon={Award}
                downloadPath={c.id ? `/exams/certificate/${c.id}/pdf` : undefined}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {reports.map((r) => (
              <DownloadCard
                ...
                note={r.remarks}
                icon={FileText}
                downloadPath={r.id ? `/exams/report/${r.id}/pdf` : undefined}
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {invoices.map((inv, i) => (
              <DownloadCard
                ...
                icon={Receipt}
                downloadPath={inv.id ? `/payments/invoice/${inv.id}` : undefined}
                url={inv.url}
              />
            ))}
          </div>

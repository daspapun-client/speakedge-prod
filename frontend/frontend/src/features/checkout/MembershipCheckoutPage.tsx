...
export function MembershipCheckoutPage() {
  ...
  // Upgrades only: when the new tier takes over. An upgrade starts as soon as
  // it is paid for unless the member deliberately schedules it, because until
  // that date the dashboard keeps showing the membership being left behind.
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await startSubscriptionCheckout({
        ...
        activate_on: upgrading && startLater && activateOn ? activateOn : undefined,
      ...
      <div className="mx-auto max-w-xl">
        <div className="card text-center">
          ...
          <p className="mt-2 text-slate-600">
            {upgrading && startLater && activateOn && activateOn !== isoDay(new Date()) ? (
              <>
        ...
        <form onSubmit={onSubmit} className="card grid gap-3 self-start">
          ...
          {upgrading && (
            <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50 p-4">
              <p className="text-sm font-bold text-amber-900">When should the upgrade start?</p>
              {/* Explicit, because the two answers look very different on the
                  dashboard: "later" leaves the tier being left in force until
                  the day, which reads as a failed upgrade if it was picked by
                  accident. Starting now is the default. */}
              <div className="mt-2 grid gap-2 text-xs text-amber-900">
                <label className="flex items-start gap-2">
                  <input
                    type="radio"
                    className="mt-0.5"
                    checked={!startLater}
                    onChange={() => { setStartLater(false); setActivateOn(''); }}
                  />
                  <span>
                    <span className="font-semibold">Start immediately</span> — your {plan.label}{' '}
                    membership is active as soon as the payment is confirmed.
                  </span>
                </label>
                <label className="flex items-start gap-2">
                  <input
                    ...
                    className="mt-0.5"
                    checked={startLater}
                    onChange={() => setStartLater(true)}
                  />
                  <span>
                    <span className="font-semibold">Start on a later date</span> — keep your{' '}
                    {quote!.previous_label} membership running until a date you choose (within{' '}
                    {quote!.activation_days} days).
                  </span>
                </label>
              </div>
              {startLater && (
              ...
              <p className="mt-3 text-sm font-bold text-amber-900">Important Upgrade Notice</p>
              <p className="mt-1 text-xs leading-relaxed text-amber-800">
                {startLater
                  ? `Your existing ${quote!.previous_label} membership and its benefits remain active until the activation date you select. From that date your existing membership is deactivated and replaced by the upgraded membership.`
                  : `Your existing ${quote!.previous_label} membership is deactivated and replaced by the ${plan.label} membership as soon as this payment is confirmed.`}{' '}
                Any benefits or entitlements already used under your existing membership will be
                deducted from the corresponding benefits available under the upgraded membership.
              </p>
            </div>
          )}

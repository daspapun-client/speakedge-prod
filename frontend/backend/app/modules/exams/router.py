...
# Exam entitlement is granted by the membership tier alone — there is no
# complimentary test on top of it. A blanket floor of one test per kind used to
# live here, which handed Tribe (1 Speaking Test, no CEFR test) two tests.
BASE_ELIGIBILITY = {"CEFR": 0, "Speaking": 0}
...
def _exam_json(exam: Exam, *, meeting: bool = False) -> dict:
    data = exam.model_dump(mode="json")
    data["scheduled_at"] = _iso(exam.scheduled_at)
    if not meeting:
        # The meeting room belongs to the people actually sitting the exam —
        # admin, the assigned examiner and the students holding a seat. A slot
        # in the picker must not hand every signed-in learner someone else's
...
async def _future_slots(rule_id: str) -> list[Exam]:
    """The upcoming dates a weekly slot has already produced."""
    now = utcnow().replace(tzinfo=None)
    return [e for e in await Exam.find({"rule_id": rule_id, "is_archived": False}).to_list()
            if e.scheduled_at and _naive(e.scheduled_at) >= now]


async def _push_meeting_url(rule: ExamSlotRule) -> None:
    """Carry a weekly slot's link onto the dates it has already generated."""
    for exam in await _future_slots(str(rule.id)):
        if exam.meeting_url == rule.meeting_url:
            continue
        exam.meeting_url = rule.meeting_url
        exam.touch()
        await exam.save()
        if exam.meeting_url:
            await _notify_meeting_link(exam)


async def _notify_meeting_link(exam: Exam) -> None:
    """Tell everyone holding a seat that the joining link is ready."""
    ...
    await notif.notify_all(
        [b.student_id for b in seats], "Exam Meeting Link Ready",
        f"The joining link for your {exam.kind} test '{exam.title}'"
        f"{_when(exam.scheduled_at)} is ready. Open Exams & Certification to join.",
...
def _slot_row(exam: Exam, examiners: dict[str, User], taken: int,
              meeting: bool = False) -> dict:
    """One bookable slot exactly as the spec lists it."""
    data = _exam_json(exam, meeting=meeting)
...
async def _slot_rows(exams: list[Exam], meeting: bool = False) -> list[dict]:
    ...
    return [_slot_row(e, examiners, taken.get(str(e.id), 0), meeting) for e in exams]
...
async def _eligibility(student_id: str) -> dict:
    ...
    if sub:
        # The tier's own totals, read live off the catalogue so an admin edit
        # (or a spec revision) applies to memberships already sold; the values
        # frozen on the Subscription are the fallback.
        cfg = await PlanConfig.find_one(PlanConfig.plan == sub.plan)
        allowed["CEFR"] = cfg.cefr_tests if cfg else sub.cefr_tests
        allowed["Speaking"] = cfg.speaking_tests if cfg else sub.speaking_tests
...
class SlotRuleBody(BaseModel):
    ...
    meeting_url: str | None = None  # Google Meet link, copied onto every date
...
async def create_slot_rule(body: SlotRuleBody, admin: CurrentUser = Depends(require_admin)):
    ...
    rule = ExamSlotRule(**{**body.model_dump(), "title": body.title.strip(),
                           ...
                           "meeting_url": _clean_meeting_url(body.meeting_url)})
    await rule.insert()
...
async def update_slot_rule(rule_id: str, body: SlotRulePatch,
    ...
    if "meeting_url" in data:
        data["meeting_url"] = _clean_meeting_url(data["meeting_url"])
    ...
    if "meeting_url" in data:
        # rebuild() only re-cuts the *unbooked* dates, so a new link has to be
        # pushed onto the ones students already hold a seat on as well.
        await _push_meeting_url(rule)
    ...
    for date_str in body.dates:
        for time_str in body.times:
            ...
            exam = Exam(
                ...
                examiner_id=body.examiner_id, meeting_url=meeting_url,
            )
            await exam.insert()
            created.append(exam)
...
async def list_exams(user: CurrentUser = Depends(get_current_user),
    ...
    rows = await _slot_rows(exams, meeting=user.is_admin or user.role == Role.examiner)
    if bookable:
...
async def update_exam(exam_id: str, body: ExamPatch,
    ...
    await log_activity(admin.subject, "exam.slot_update", role=admin.role.value,
                       ...
                       meta={"fields": sorted(data.keys())})
    return ok(_exam_json(exam, meeting=True))
...
class MeetingLinkBody(BaseModel):
    meeting_url: str | None = None  # null / blank clears it


@router.post("/{exam_id}/meeting-link")
async def set_meeting_link(exam_id: str, body: MeetingLinkBody,
                           user: CurrentUser = Depends(get_current_user)):
    """Set the meeting link the exam is conducted on (Google Meet or similar).

    Admin sets it from the slot list; the examiner conducting the sitting can
    paste their own room in from the examiner dashboard. The examiner check is
    inline rather than a portal guard on purpose — this one endpoint is shared
    by the two portals, and an examiner may only touch their own slots."""
    exam = await Exam.get(exam_id)
    if not exam or exam.is_archived:
        raise NotFoundError("Exam not found")
    if not user.is_admin and not (user.role == Role.examiner
                                  and exam.examiner_id == user.subject):
        raise ForbiddenError("This exam slot is not assigned to you")
    exam.meeting_url = _clean_meeting_url(body.meeting_url)
    exam.touch()
    await exam.save()
    if exam.meeting_url:
        await _notify_meeting_link(exam)
    await log_activity(user.subject, "exam.meeting_link", role=user.role.value,
                       target_type="exam", target_id=exam_id,
                       meta={"set": bool(exam.meeting_url)})
    return ok(_exam_json(exam, meeting=True),
              "Meeting link updated" if exam.meeting_url else "Meeting link removed")
...
async def admin_bookings(admin: CurrentUser = Depends(require_admin),
    ...
    for b in items:
        ...
        data["meeting_url"] = exam.meeting_url if exam else None
    ...
    for b in items:
        ...
        out.append({
            **b.model_dump(mode="json"),
            ...
            "meeting_url": exam.meeting_url if exam else None,
            **_examiner_fields(examiners.get(exam.examiner_id or "") if exam else None),
            "result": ({"type": "cefr_report", "id": str(report.id),
                        "level": report.level,
                        "remarks": report.remarks, "scores": report.scores,
                        "url": report.report_url,
                        "verification_code": report.verification_code}
                       if report else
                       {"type": "certificate", "id": str(cert.id), "grade": cert.grade,
                        "remarks": cert.remarks, "url": cert.certificate_url,
                        "title": cert.title, "level": cert.cefr_level,
...
async def _certificate_json(cert: Certificate) -> dict:
    """A certificate as the certificate page renders it — the candidate name,
    the assessment panel, the certificate number and the CEFR level awarded.

    Certificates issued before the name and level were frozen on the document
    fall back to the profile, so an older one still renders in full."""
    name, level = cert.student_name, cert.cefr_level
    if not (name and level):
        student = await Student.find_one(Student.student_id == cert.student_id)
        name = name or (student.full_name if student else None)
        level = level or (student.cefr_level if student else None)
    return {
        "id": str(cert.id),
        "student_name": name,
        "student_id": cert.student_id,
        "assessment_date": _iso(cert.exam_date),
        "issue_date": _iso(cert.issued_at),
        "certificate_no": cert.verification_code,
        "cefr_level": level,
        "title": cert.title,
        "grade": cert.grade,
...
async def certificate_detail(certificate_id: str,
                             user: CurrentUser = Depends(require_student)):
    """One certificate, student-scoped — another learner's is simply not found."""
    cert = await Certificate.get(certificate_id)
    if not cert or cert.student_id != user.subject:
        raise NotFoundError("Certificate not found")
    return ok(await _certificate_json(cert))


@router.get("/certificate/{certificate_id}/pdf")
async def certificate_pdf(certificate_id: str,
                          user: CurrentUser = Depends(require_student)):
    """Regenerate the certificate PDF from the stored record — survives a missing
    /media file after a redeploy."""
    cert = await Certificate.get(certificate_id)
    if not cert or cert.student_id != user.subject:
        raise NotFoundError("Certificate not found")
    ...
    if not name:
        student = await Student.find_one(Student.student_id == cert.student_id)
        name = student.full_name if student else cert.student_id
    pdf = pdf_service.certificate_pdf_bytes(
        student_name=name,
        student_id=cert.student_id,
        verification_code=cert.verification_code,
        cefr_level=cert.cefr_level,
        assessment_date=cert.exam_date,
        issue_date=cert.issued_at,
    )
    name = cert.verification_code or certificate_id
    return Response(
...
async def _report_json(report: CEFRReport) -> dict:
    """A CEFR report card as the result-card page renders it — the candidate
    name, the assessment panel and the level awarded.

    The report freezes no candidate name, so it is read off the profile."""
    student = await Student.find_one(Student.student_id == report.student_id)
    return {
        "id": str(report.id),
        "student_name": student.full_name if student else None,
        "student_id": report.student_id,
        "exam_date": _iso(report.exam_date),
        "issue_date": _iso(report.created_at),
        "cefr_level": report.level,
        "report_no": report.verification_code,
...
async def report_detail(report_id: str,
                        user: CurrentUser = Depends(require_student)):
    """One CEFR report card, student-scoped — another learner's is not found."""
    report = await CEFRReport.get(report_id)
    if not report or report.student_id != user.subject:
        raise NotFoundError("Report card not found")
    return ok(await _report_json(report))


@router.get("/report/{report_id}/pdf")
async def report_pdf(report_id: str,
                     user: CurrentUser = Depends(require_student)):
    """Regenerate the CEFR report PDF from the stored record — survives a missing
    /media file after a redeploy."""
    report = await CEFRReport.get(report_id)
    if not report or report.student_id != user.subject:
        raise NotFoundError("Report card not found")
    student = await Student.find_one(Student.student_id == report.student_id)
    name = student.full_name if student else report.student_id
    pdf = pdf_service.cefr_report_pdf_bytes(
        name, report.student_id, report.level, report.scores or {},
        report.verification_code,
    )
    fname = report.verification_code or report_id
    return Response(
        content=pdf, media_type="application/pdf",
...
async def book(exam_id: str, user: CurrentUser = Depends(require_student)):
    ...
    quota = (await _eligibility(user.subject)).get(exam.kind, {"allowed": 0, "remaining": 0})
    if quota["remaining"] <= 0:
        raise ConflictError(
            f"The {exam.kind} test is not included in your membership. "
            "Upgrade your membership to book one."
            if quota.get("allowed", 0) <= 0 else
    ...
    where = (" The joining link is on your Exams & Certification page."
             if exam.meeting_url else "")
    await notif.notify(user.subject, "Exam Booking Confirmed",
                       f"Your {exam.kind} test '{exam.title}'{when} "
                       f"({exam.duration_minutes} min) has been booked.{who}{where}",
                       kind="exam", push_url="/dashboard/exams")
    if exam.examiner_id:
...
class ReportBody(BaseModel):
    ...
    cefr_level: str | None = None  # A1..C2 printed on the Speaking certificate
    scores: dict = {}
...
async def submit_report(body: ReportBody, examiner: CurrentUser = Depends(require_examiner)):
    ...
    if exam.kind == "CEFR":
        ...
        # The certificate is CEFR-aligned, so it prints a level: the one the
        # examiner awards here, or the learner's verified level when they leave
        # it blank.
        cefr_level = (body.cefr_level or "").strip().upper() or None
        if cefr_level and cefr_level not in CEFR_LEVELS:
            raise ValidationAppError("CEFR level must be one of A1, A2, B1, B2, C1, C2")
        ccode = "CERT-" + uuid.uuid4().hex[:10].upper()
        cert = Certificate(
            title=f"Speaking Test Certificate – {body.grade}", grade=body.grade,
            student_name=name, cefr_level=cefr_level or (student.cefr_level if student else None),
            verification_code=ccode, issued_at=utcnow(), **shared,
        )
        cert.certificate_url = pdf_service.generate_certificate(
            student_name=name,
            student_id=body.student_id,
            verification_code=ccode,
            cefr_level=cert.cefr_level,
            assessment_date=exam.scheduled_at,
            issue_date=cert.issued_at,
        )
        await cert.insert()
...
# Anyone holding a printed certificate or report card — an employer, a college,
# an institute — checks it here, with no SpeakEdge account. The payload is
# deliberately limited to what is already printed on the document the enquirer
# is holding (name, student id, level/grade, dates, examiner), so verification
# confirms a document rather than disclosing a learner's record.
async def _candidate_name(doc: Certificate | CEFRReport) -> str | None:
    """The candidate's name as printed. Frozen on the certificate at issue;
    older rows (and every CEFR report) fall back to the profile."""
    name = getattr(doc, "student_name", None)
    if not name:
        student = await Student.find_one(Student.student_id == doc.student_id)
...
async def public_verify(code: str):
    """Verify a certificate / CEFR report by the code printed on it."""
    code = code.strip().upper()

    cert = await Certificate.find_one(Certificate.verification_code == code)
    if cert:
        if cert.is_archived:
            return ok({"valid": False, "code": code, "withdrawn": True},
                      "This certificate has been withdrawn and is no longer valid.")
        level = cert.cefr_level
        if not level:
            student = await Student.find_one(Student.student_id == cert.student_id)
            level = student.cefr_level if student else None
        return ok({"type": "certificate", "valid": True, "code": code,
                   "student_name": await _candidate_name(cert),
                   "student_id": cert.student_id,
                   "title": cert.title, "grade": cert.grade, "cefr_level": level,
                   "exam_title": cert.exam_title,
                   "examiner_name": cert.examiner_name,
                   "remarks": cert.remarks,
                   "exam_date": _iso(cert.exam_date),
    ...
    if report:
        if report.is_archived:
            return ok({"valid": False, "code": code, "withdrawn": True},
                      "This report has been withdrawn and is no longer valid.")
        return ok({"type": "cefr_report", "valid": True, "code": code,
                   "student_name": await _candidate_name(report),
                   "student_id": report.student_id, "level": report.level,
                   "scores": report.scores,
                   "exam_title": report.exam_title,
                   "examiner_name": report.examiner_name,
                   "remarks": report.remarks,
    ...
    return ok({"valid": False, "code": code},

...
@router.post("/teacher/batches/{batch_id}/meeting-link")
async def teacher_meeting_link(batch_id: str, body: MeetingLink,
                               user: CurrentUser = Depends(require_role(Role.teacher))):
    """The assigned teacher shares (or clears) their session's join link.

    Mirrors ``POST /teacher/batches/{id}/meeting-link`` on the teacher module:
    the teacher creates the meeting themselves, so they must be able to publish
    the link without waiting for admin. Students read it off their own
    orientation page, so setting it notifies everyone enrolled."""
    batch = await OrientationBatch.get(batch_id)
    if not batch or batch.is_archived:
        raise NotFoundError("Orientation session not found")
    t = await _own_teacher(user)
    if batch.teacher_id != str(t.id):
        raise ForbiddenError("This orientation session is not assigned to you")
    if batch.mode != "live":
        raise ValidationAppError("Only a live session has a join link")
    url = (body.meeting_url or "").strip() or None
    if url and not url.startswith(("http://", "https://")):
        raise ValidationAppError("Enter the full link, starting with https://")
    batch.meeting_url = url
    batch.touch()
    await batch.save()
    await log_activity(user.subject, "orientation.meeting_link", role=user.role.value,
                       target_type="orientation_batch", target_id=str(batch.id),
                       meta={"set": bool(url)})
    if url and batch.student_ids:
        await notif.notify_all(
            list(batch.student_ids), "Orientation link ready",
            f'The join link for your orientation "{batch.title}" is now available.',
            "info", push_url="/dashboard/orientation")
    summary = await _batch_summary(batch)
    summary["roster"] = await service.roster_for(batch)
    return ok(summary, "Join link updated" if url else "Join link removed")

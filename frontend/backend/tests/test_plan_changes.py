...
async def test_a_due_upgrade_is_applied_when_the_student_looks(client):
    """The dashboard must never show the tier the member has moved off. The
    scheduler runs hourly and only while the process is up, so the read itself
    switches a membership over once its activation date has passed."""
    student = await _student_headers(client)
    await _plans(client)
    await _subscribe("Silver")
    when = (datetime.now(timezone.utc) + timedelta(days=5)).strftime("%Y-%m-%d")
    await _pay(client, student, plan="Gold", activate_on=when)
    ...
    # No scheduler pass in between: /subscription/current and the dashboard
    # both resolve the membership through payments.service.
    assert (await _current(client, student))["plan"] == "Gold"
    home = (await client.get("/api/v1/dashboard/", headers=student)).json()["data"]
    assert home["subscription"]["plan"] == "Gold"
    assert home["subscription"]["pending_plan"] is None
    assert await pay.apply_due_plan_changes() == 0   # nothing left for the sweep


async def test_a_second_upgrade_cannot_be_paid_for_while_one_is_pending(client):
    """The credit is taken from the membership in force, which is still the old
    tier until the activation date — so a second purchase would be priced
    against it and then discarded. A member who thinks the first one failed must
    ...
    r = await client.post("/api/v1/payments/order", headers=student, json={
        "plan": "Diamond", "months": 12, "accept_terms": True,
    })
    assert r.status_code == 409
    assert "already paid for" in r.json()["error"]["message"]

...
import {
  DEFAULT_SLOT_MINUTES, EXAM_KINDS, ExaminerContact, JoinMeeting, WEEKDAYS, fmtSlot,
  slotWindow, weeklySlotLabel, type ExamSlot, type ExamSlotRule, type ExaminerRow,
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Where the exam is conducted. It is copied onto every date this slot
              produces — including dates students have already booked — and the
              examiner can replace it with their own room from their dashboard.
            </p>
          </div>
        </div>
...
function WeeklySlotsTab({ examiners, refresh }: { examiners: ExaminerRow[]; refresh: () => void }) {
  ...
  const columns: Column<ExamSlotRule>[] = [
    ...
    { key: 'examiner', header: 'Examiner', sort: (r) => r.examiner_name ?? '', cell: (r) => <ExaminerContact examiner={r} /> },
    {
      key: 'meeting',
      header: 'Meeting',
      cell: (r) => (r.meeting_url
        ? <JoinMeeting url={r.meeting_url} className="btn-ghost inline-flex items-center gap-1.5 py-1 text-xs" />
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <input className="input" placeholder="https://meet.google.com/…"
              ...
              onChange={(e) => setForm({ ...form, meeting_url: e.target.value })} />
            <p className="mt-1 text-xs text-slate-400">
              This date only. Students holding a seat are notified when it changes.
            </p>
          </div>
        </div>
...
function SlotsTab({ examiners, refresh }: { examiners: ExaminerRow[]; refresh: () => void }) {
  ...
  const columns: Column<ExamSlot>[] = [
    ...
    { key: 'examiner', header: 'Examiner', sort: (s) => s.examiner_name ?? '', cell: (s) => <ExaminerContact examiner={s} /> },
    {
      key: 'meeting',
      header: 'Meeting',
      cell: (s) => (s.meeting_url
        ? <JoinMeeting url={s.meeting_url} className="btn-ghost inline-flex items-center gap-1.5 py-1 text-xs" />
        : <span className="text-xs text-slate-400">Not set</span>),
    },
    {
      key: 'seats',

...
async def assign_students(batch_id: str, body: AssignStudents,
    ...
    batch.touch()
    await batch.save()
    # A seat is sold by the cycle one monthly fee buys, not by the sitting, so
    # adding a student to one date of a course enrols them for the next
    # SESSIONS_PER_CYCLE classes — exactly what an approved join request does.
    sessions = 1
    if batch.series_id:
        for sid in added:
            n = len(await enrol.top_up(batch.series_id, sid)) + 1  # + the one added above
            sessions = max(sessions, min(n, enrol.SESSIONS_PER_CYCLE))
        batch = await Batch.get(batch_id) or batch  # top_up saved its own copies
    if added:
        await _log_batch(admin, "batch.students_add", batch_id,
                         student_ids=added, count=len(added),
                         series_id=batch.series_id, sessions=sessions)
        await notif.notify_all(added, "Added to Batch",
                               f"You have been enrolled in \"{batch.title}\" for your next "
                               f"{sessions} class(es). Your seat continues each week — "
                               "no need to request again."
                               if batch.series_id else
...
async def remove_student(batch_id: str, body: MemberRef,
    ...
    if not batch:
        raise NotFoundError("Batch not found")
    # Off the course from here on, not off one sitting — a student left seated
    # in the cycle's other three classes is the same bug in reverse. Past
    # sittings keep the roster they were taught with.
    if batch.series_id:
        removed = bool(await enrol.remove(batch.series_id, body.student_id))
        batch = await Batch.get(batch_id) or batch
    else:
...
async def request_join(batch_id: str, user: CurrentUser = Depends(require_unlocked_teacher_student)):
    ...
    # One request covers the whole cycle. A course repeats weekly, so asking
    # again every week is exactly what the spec rules out: the request is filed
    # against the next SESSIONS_PER_CYCLE sittings at once and admin decides it
    # once (see app/shared/batch_enrolment.py).
    if batch.series_id:
        covered = await enrol.request(batch.series_id, user.subject)
        if not covered:
            raise ConflictError("Your join request is already pending")
    else:
    ...
    await _log_batch(user, "batch.join_request", batch_id, student_id=user.subject,
                     series_id=batch.series_id, sessions=len(covered))
    await _notify_batch(batch, "New Batch Join Request",
                        f"A student requested to join \"{batch.title}\" "
                        f"({len(covered)} class(es)) — awaiting admin approval.",
                        "approval", include_students=False)
    return ok({"sessions": len(covered),
               "session_dates": [enrol.session_date(b) for b in covered]},
              f"Join request sent for your next {len(covered)} class(es). "
...
async def withdraw_join(batch_id: str, user: CurrentUser = Depends(require_unlocked_teacher_student)):
    ...
    if not batch or batch.is_archived:
        raise NotFoundError("Batch not found")
    if batch.series_id:
        # Withdraw the one request, i.e. off every sitting it was filed against.
        if not await enrol.withdraw(batch.series_id, user.subject):
...
async def approve_join(batch_id: str, body: MemberRef,
    ...
    if not batch:
        raise NotFoundError("Batch not found")
    # One decision, applied across the cycle the student asked for — they are
    # enrolled for those consecutive classes and do not request again weekly.
    if batch.series_id:
        covered = await enrol.decide(batch.series_id, body.student_id, approve=True)
        if not covered:
            raise NotFoundError("No such pending request")
        # enrol.decide saved its own copies — take the fresh one back.
        batch = next((b for b in covered if str(b.id) == batch_id), batch)
    else:
        ...
        await batch.save()
        covered = [batch]
    dates = [enrol.session_date(b) for b in covered]
    await _log_batch(user, "batch.join_approve", batch_id, student_id=body.student_id,
                     series_id=batch.series_id, sessions=len(covered), session_dates=dates)
    await notif.notify(body.student_id, "Batch Join Approved",
                       f"You've been enrolled in \"{batch.title}\" for your next "
                       f"{len(covered)} class(es)"
                       + (f" ({dates[0]} – {dates[-1]})." if dates else ".")
                       + " Your seat continues each week — no need to request again.",
                       kind="approval")
...
async def reject_join(batch_id: str, body: MemberRef,
    ...
    if not batch:
        raise NotFoundError("Batch not found")
    rejected = (await enrol.decide(batch.series_id, body.student_id, approve=False)
                if batch.series_id else None)
    if rejected or (rejected is None and body.student_id in batch.pending_ids):
        if rejected is None:
            batch.pending_ids.remove(body.student_id)
            batch.touch()
            await batch.save()
...
async def browse_batches(user: CurrentUser = Depends(require_unlocked_teacher_student)):
    ...
    for sid in series_ids:
        ...
        if s and not s.is_archived:
            stitles[sid] = s.title
    # A course repeats weekly, so the student must see (and act on) the course,
    # not each dated sitting: group the siblings up once and hang the
    # series-wide view off every row.
    by_series: dict[str, list[Batch]] = {}
    for b in batches:
        if b.series_id:
    ...
    for b in batches:
        ...
        siblings = by_series.get(b.series_id or "", [b])
        upcoming = enrol.upcoming(siblings)
        rows.append({
            "id": str(b.id), "title": b.title,
            "series_id": b.series_id,
            "series_title": stitles.get(b.series_id) if b.series_id else None,
            # The course as a whole: how the browse card decides join vs pending
            # vs "already enrolled", and what it promises the request covers.
            "series_status": enrol.series_status(siblings, user.subject),
            "series_session_dates": [enrol.session_date(x) for x in upcoming],
            "my_session_dates": sorted(
                enrol.session_date(x) for x in upcoming
                if user.subject in x.student_ids or user.subject in x.pending_ids
            ),
            "sessions_per_cycle": enrol.SESSIONS_PER_CYCLE,
    ...
    return ok({"batches": rows, "batches_used": used, "batch_limit": limit,
               # One request buys this many consecutive weekly classes — the
               # same cycle a monthly fee covers (app/shared/batch_enrolment.py).
               "sessions_per_cycle": enrol.SESSIONS_PER_CYCLE,
...
async def _batches_for_teacher(t: Teacher) -> list[dict]:
    ...
    if batch_ids:
        for att in await Attendance.find(
            ...
            att_dates.setdefault(att.batch_id, []).append(att.date)
    stitles: dict[str, str] = {}
    # Sittings of each course, so a roster row can show the whole cycle the
    # student is enrolled for — one monthly fee buys SESSIONS_PER_CYCLE classes,
    # and the teacher plans against those, not against this one date.
    ssessions: dict[str, list[Batch]] = {}
    for sid in {b.series_id for b in batches if b.series_id}:
        s = await BatchSeries.get(sid)
        if s and not s.is_archived:
            stitles[sid] = s.title
        ssessions[sid] = await enrol.sessions(sid)
    out = []
    for b in batches:
        siblings = ssessions.get(b.series_id or "", [])
        upcoming_dates = [enrol.session_date(x) for x in enrol.upcoming(siblings)]
        ...
        for sid in b.student_ids:
            ...
            students.append({
                ...
                # The classes this seat still covers on this course (empty for a
                # loose batch, which is only ever the one sitting).
                "session_dates": enrol.enrolled_dates(siblings, sid),
            })
        data = b.model_dump(mode="json")
        data["students"] = students
        data["series_title"] = stitles.get(b.series_id) if b.series_id else None
        data["series_session_dates"] = upcoming_dates
        data["sessions_per_cycle"] = enrol.SESSIONS_PER_CYCLE
        data["pending"] = [
            {**p, "session_dates": [enrol.session_date(x) for x in enrol.upcoming(siblings)
                                    if p["student_id"] in x.pending_ids]}
            for p in await _resolve_names(b.pending_ids)
        ]

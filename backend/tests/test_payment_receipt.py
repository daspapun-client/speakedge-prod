...
async def test_invoice_download_survives_a_missing_media_file(client):
    """The /media copy is gone after a redeploy; the download must still work."""
    ...
    await client.post("/api/v1/payments/verify", headers=student, json={
        "razorpay_order_id": order["order_id"], "razorpay_payment_id": "pay_test_inv1",
        "razorpay_signature": "test",
    })

    payment = await Payment.find_one(Payment.razorpay_order_id == order["order_id"])
    assert payment and payment.invoice_url
    disk = Path(file_service.media_root()) / payment.invoice_url.removeprefix("/media/")
    assert disk.is_file()
    disk.unlink()

    r = await client.get(f"/api/v1/payments/invoice/{payment.id}", headers=student)
    assert r.status_code == 200, r.text
    assert r.content.startswith(b"%PDF")
    ...
    other = await _student_headers(client, student_id="SPK-26-OTHER1")
    assert (await client.get(f"/api/v1/payments/invoice/{payment.id}",
